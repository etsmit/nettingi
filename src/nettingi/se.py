        ...
        for pol in range(a.shape[3]):
            for chan in tqdm(range(a.shape[0])):
                for tb in range(a.shape[1]):
                    H[chan, tb, pol], Hbase[chan, tb, pol] = getHs(
                        a[chan, tb, :, pol], nbit_range
                    )
        print("sre")
...
def getHs(a, bins):
    ...
    outH = np.sum(-pdfr * np.log(pdfr)) + 1.0j * np.sum(-pdfi * np.log(pdfi))
    outHbase = (0.5 * (1 + np.log(2 * np.pi * np.var(a.real)))) + 1.0j * (
        0.5 * (1 + np.log(2 * np.pi * np.var(a.imag)))
    )
