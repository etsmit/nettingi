...
class rfi_sk(mitigateRFI):
        ...
        if self.mssk != "1,1":
            ...
            for ichan in range(self.ms0):
                for itime in range(self.ms1):
                    flags_block[
                        ichan : ichan + (num_coarsechan - (self.ms0 - 1)),
                        itime : itime + (num_SKbins - (self.ms1 - 1)),
                        :,
                    ][ms_flags_block == 1] = 1
        else:
    ...
    def multi_scale_SK_EST(self, s):
        ...
        for ichan in range(self.ms0):
            for itime in range(self.ms1):
                ms_s1 += (1.0 / ms_binsize) * (
                    s1[
                        ichan : ichan + (s.shape[0] - (self.ms0 - 1)),
                        itime : itime + (num_SKbins - (self.ms1 - 1)),
                        :,
                    ]
                )
                ms_s2 += (1.0 / ms_binsize) * (
                    s2[
                        ichan : ichan + (s.shape[0] - (self.ms0 - 1)),
                        itime : itime + (num_SKbins - (self.ms1 - 1)),
                        :,
    ...
    def upperRoot(self, x, moment_2, moment_3, p):
        upper = np.abs(
            (
                1
                - sp.special.gammainc(
                    (4 * moment_2**3) / moment_3**2,
                    (-(moment_3 - 2 * moment_2**2) / moment_3 + x)
                    / (moment_3 / 2 / moment_2),
                )
            )
            - p
    ...
    def SK_thresholds(self, M):
        ...
        lowerThreshold = sp.optimize.newton(
            self.lowerRoot, x[0], args=(moment_2, moment_3, p)
        )
    ...
    def check_m(self, m, tlen):
        ...
        if mismatch != 0:
            print(
                f"There are {tlen} time samples and you set M = {m}, you will miss {mismatch} time samples at the end of each block"
            )
            print(
                f"This corresponds to {np.around(100*mismatch/tlen,4)}% of your data."
            )
