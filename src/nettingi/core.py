...
class mitigateRFI:
    """
    Top-level class for RFI mitigation.
    ...
    def run_all(self):
        ...
        for bi in range(numblocks // self.mb):
            ...
            if self.repl_method == "previousgood":
                ...
                print(
                    "Cannot replace with previous good data currently, switching to statistical noise"
                )
                data[:, :rlen, :] = statistical_noise_fir(
                    data[:, :rlen, :], flags_block, ts_factor
                )
        ...
        if self.det_method == "SK":
            ...
            os.system(
                f"""echo "'{self._spect_filename}','{self._flags_filename}','{self._regen_filename}','{self._ss_sk_filename}','{self._ms_sk_filename}'\n===============================" >> {log}"""
            )
            ...
            os.system(
                f"""echo "'{self._spect_filename}','{self._flags_filename}','{self._regen_filename}'\n===============================" >> {log}"""
            )
            ...
            os.system(
                f"""echo "'{self._spect_filename}','{self._flags_filename}','{self._regen_filename}','{self._zsc_filename}'\n===============================" >> {log}"""
            )
        ...
        # np.save(self._avg_post_filename, self.avg_post)
            ...
            os.system(
                f"""echo "'{self._spect_filename}','{self._flags_filename}','{self._regen_filename}','{self._ptest_filename},'{self._stat_filename}'\n===============================" >> {log}"""
            )
    ...
    def fine_channelize(self, resolution, mit=False, mask=False):
        ...
        if mit:
            if mask:
                ...
                raw2spec_god(
                    resolution,
                    GuppiRaw(self.outfile_raw_full),
                    self.det_method,
                    out_fc_fname,
                    in_mask,
                )
            ...
            if mask:
                ...
                raw2spec_god(
                    resolution, self._rawFile, self.det_method, out_fc_fname, in_mask
                )
            else:
    ...
    def load_basic_srdps(self):
        ...
        if self.det_method == "AOF":  # or (self.det_method == 'MAD'):
            f = load_raw_flags(
                self.output_srdp_dir + self.npybase + "_flags_block*.npy",
                self.ave_factor,
            )
            out = out + (f,)
        else:
