...
class rfi_iqrm(mitigateRFI):
    ...
    def __init__(
        ...
        repl_method,
        IQRM_radius_time=5,
        IQRM_threshold_time=3.0,
        IQRM_radius_freq=5,
        IQRM_threshold_freq=3.0,
        IQRM_datatype="std",
        two_d=False,
        IQRM_breakdown=512,
        cust="",
        ...
        if self.two_d:
            ...
            self._outfile_pattern = (
                f"r{IQRM_radius_time}_t{IQRM_threshold_time}_"  # {IQRM_datatype}'
            )
    ...
    def iqrm_detection(self, data):
        ...
        if self.IQRM_datatype == "power":
            ...
            flag_chunk = iqrm_avgpwr(
                data,
                self.IQRM_radius_time,
                self.IQRM_threshold_time,
                self.IQRM_radius_freq,
                self.IQRM_threshold_freq,
                self.IQRM_breakdown,
                self.two_d,
            )
...
def iqrm_std(
    data, radius_time, threshold_time, radius_freq, threshold_freq, breakdown, two_d
):
    """
    ...
    for i in tqdm(range(data.shape[2])):  # iterate through polarizations
        for j in range(data.shape[0]):  # iterate through channels
            flag_chunk[j, :, i] = iqrm.iqrm_mask(
                data[j, :, i], radius=radius_time, threshold=threshold_time
            )[0]

        if two_d:
            for j in range(data.shape[1]):
                flag_chunk[:, j, i] = iqrm.iqrm_mask(
                    data[:, j, i], radius=radius_freq, threshold=threshold_freq
                )[0]
