...
def pfb_mask(
    x, nchan, ntap, window="hann", fs=1.0, return_freqs=False, force_complex=False
):
    """
    ...
    for ii in range(ntap * (nwin - 1) + 1):
        xw = np.ma.masked_array(
            h * x[:, ii : ii + ntap], np.ma.getmask(x[:, ii : ii + ntap])
        )
...
def pfb(x, nchan, ntap, window="hann", fs=1.0, return_freqs=False, force_complex=False):
    """
    Channelize data using a polyphase filterbank
...
def raw2spec(resolution, gr, infile):
    ...
    print(basenm)
    outfile_test = os.path.join(
        "/data/scratch/IQRMresults", basenm.replace(".raw", f".{resolution}.spec.pkl")
    )
    print("hi")
    ...
    print(
        f"given output res is {resolution} kHz, actual will be {np.abs(chanbw/nchan_pfb)*1e3} khz"
    )
    ...
    for bb in range(gr.n_blocks):
        ...
        for nn in range(data.shape[0]):
            ...
            spec = (
                np.mean(np.abs(xpfb) ** 2, axis=0) + np.mean(np.abs(ypfb) ** 2, axis=0)
            ) / 2
    ...
    with open(
        os.path.join(
            "/data/scratch/IQRMresults",
            basenm.replace(".raw", f".{resolution}.spec.pkl"),
        ),
        "wb",
    ) as f:
        pickle.dump(out1, f)
...
# do fine channelization using a mask
def raw2spec_god_cupy(resolution, gr, det, outfile, mask=None):
