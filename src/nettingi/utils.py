...
def template_parse(parser):
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='String. Required. Name of input filename. Automatically pulls from standard data directory. If leading "/" given, pulls from given directory',
    )
    ...
    parser.add_argument(
        ...
        dest="method",
        type=str,
        choices=["zeros", "previousgood", "stats", "nans"],
        required=True,
        default="zeros",
        help='String. Required. Replacement method of flagged data in output raw data file. Can be "zeros","previousgood", nans or "stats"',
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=True,
        help="Copy the original data and output a replaced datafile. Default True. Change to False to not write out a whole new GUPPI file",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="custom tag to add to end of filename",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="load multiple blocks at once to help with stats/prevgood replacement",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Combine the polarizations in the flagging step. Default 1.",
    )
...
def template_infile_mod(infile, in_dir):
    ...
    if infile[-4:] != ".raw":
        input(
            "WARNING input filename doesn't end in '.raw'. Are you sure you want to use this file?"
        )
    return infile
...
def template_bookkeeping(infile, out_patt, det):
    ...
    output_mit_srdp_dir = (
        f"{output_mit_srdp_dir_base}/{output_base}/{infile[:-4]}_{det}_{out_patt}/"
    )
...
def template_set_up_pulsar_reduction(output_mit_srdp_dir, infile_base):
    ...
    if psr == "J1713":
        os.system(
            f"ln -sf {psr_redux_dir}parfiles/J1713+0747.par {output_mit_srdp_dir}"
        )
    if psr == "B0329":
...
def template_check_nblocks(rawFile, mb):
    ...
    if mismatch != 0:
        print(
            f"There are {numblocks} blocks and you set -mb {mb}, pick a divisible integer"
        )
        sys.exit()
    ...
    for pol in prange(f.shape[2]):
        for i in prange(f.shape[0]):
            ...
            if bad_data_size > 0:
                ...
                # print('--------------------')
                a[i, :, pol].real[f[i, :, pol] == 1] = noise_filter(
                    0, std_real, bad_data_size, dec
                )[:bad_data_size]
                a[i, :, pol].imag[f[i, :, pol] == 1] = noise_filter(
                    0, std_imag, bad_data_size, dec
                )[:bad_data_size]
    return a
        ...
        for i in prange(f.shape[0]):
            for tb in prange(f.shape[1] // SK_M):
                if f[i, SK_M * tb, pol] == 1:
                    ...
                    ave_real, ave_imag, std_real, std_imag = adj_chan_good_data_alt(
                        a[:, tb * SK_M : (tb + 1) * SK_M, pol],
                        f[:, tb * SK_M : (tb + 1) * SK_M, pol],
                        i,
                        SK_M,
                        tb,
                    )
                    # print('generating representative awgn..')
                    (a[i, tb * SK_M : (tb + 1) * SK_M, pol].real)[
                        f[i, tb * SK_M : (tb + 1) * SK_M, pol] == 1
                    ] = noise_filter(ave_real, std_real, SK_M, dec)

                    (a[i, tb * SK_M : (tb + 1) * SK_M, pol].imag)[
                        f[i, tb * SK_M : (tb + 1) * SK_M, pol] == 1
                    ] = noise_filter(ave_imag, std_imag, SK_M, dec)
    return a
