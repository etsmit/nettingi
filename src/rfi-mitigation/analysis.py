...
def raw2spec(resolution,gr)
    ...
    with open(os.path.join(args.outdir,basenm.replace(".raw",".20.spec.pkl")),"wb") as f: 
        pickle.dump(out1,f)
        print(f)
...
def raw2spec_mask(resolution,gr,mask):
    ...
    print(f'given output res is {args.resolution} kHz, actual will be {np.abs(chanbw/nchan_pfb)*1e3} khz')
    ...
    #out2 = (freqs,masked_spectrum)
    basenm = os.path.basename(rawfile)
    with open(os.path.join(args.outdir,basenm.replace(".raw","mask.20.spec.pkl")),"wb") as f: 
        pickle.dump(out1,f)
        print(f)
    #with open(os.path.join(args.outdir,basenm.replace(".raw",".20.spec_mask.pkl")), "wb") as f: 
    #    pickle.dump(out2,f)
