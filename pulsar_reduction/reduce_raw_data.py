...
def get_args():
    ...
    general_args.add_argument(
        "-o",
        "--basenm",
        help=("Base name to use for output " "(default=base name of input files)"),
    )
    general_args.add_argument(
        "-O", "--outdir", default=".", help="Output directory (default=%(default)s"
    )
    ...
    general_args.add_argument(
        "-d", "--dm", type=float, default=0.0, help="DM (pc/cc; default=%(default)s)"
    )
    general_args.add_argument(
        "--nbits",
        type=int,
    ...
    search_args.add_argument(
        "--no-search", action="store_true", help="Do not produce search-mode data"
    )
    search_args.add_argument(
        "-t",
        "--tint",
    ...
    search_args.add_argument(
        "--low-dm",
        help=("The lowest DM to use when searching " "(pc/cc; default=dm-20.0)"),
    )
    search_args.add_argument(
        "--dm-step",
        type=float,
        default=0.05,
        help=("DM step size to use when searching (pc/cc; " "default=%(default)s)"),
    )
    search_args.add_argument(
        "--ndms",
        type=int,
        help=("Number of DMs to use when searching " "(default=(dm-low_dm)/dm_step)"),
    )
    search_args.add_argument(
        "--max-harms",
        type=int,
        default=16,
        help=(
            "Maximum number of harmonics to sum when " "searching (default=%(default)s)"
        ),
    )
    search_args.add_argument(
        "--zmax",
        ...
        default=50,
        help=(
            "Maximum acceleration to use when "
    ...
    fold_args.add_argument(
        "--no-fold", action="store_true", help="Do not produce fold-mode data"
    )
    fold_args.add_argument(
        "-p",
        "--parfile",
        action=FullPath,
        help=("TEMPO-compatible parfile to use for " "folding"),
    )
    fold_args.add_argument(
        "-b",
        "--nbin",
        type=int,
        default=2048,
        help=("Number of pulse profile bins " "(default=%(default)s)"),
    )
    fold_args.add_argument(
        "-c", "--cal-psr", action=FullPath, help="Pulsar calibration data"
    )
    fold_args.add_argument("--fluxcal", action=FullPath, help="PSRCHIVE fluxcal data")
    fold_args.add_argument(
        "--fluxcal-on", action=FullPath, help="On-source fluxcal data"
    )
    fold_args.add_argument(
        "--fluxcal-off", action=FullPath, help="Off-source fluxcal data"
    )
    parser.add_argument(
        "rawfiles", action=FullPathAppend, nargs="+", help="RAW input files to process"
    )
...
def do_search(
    ...
    if zero_dm:
        ...
        ### TODO: CHANGE TO .fits WHEN DIGIFITS IS WORKING
        cmd = (
            "prepsubband -zerodm -o {basenm}_bary_zerodm_search_mask "
            "-mask {basenm}_rfifind.mask -lodm {low_dm} "
            "-dmstep {dm_step} -numdms {ndms:d} *search*.fil".format(**kwargs)
        )
        ret = execute(cmd, out=outfile, err=errfile)
        ...
        ### TODO: CHANGE TO .fits WHEN DIGIFITS IS WORKING
        cmd = (
            "prepsubband -zerodm -o {basenm}_bary_zerodm_search_nomask "
            "-lodm {low_dm} "
            "-dmstep {dm_step} -numdms {ndms:d} *search*.fil".format(**kwargs)
        )
        ret = execute(cmd, out=outfile, err=errfile)
    ...
    if make_topocentric:
        ...
        ### TODO: CHANGE TO .fits WHEN DIGIFITS IS WORKING
        cmd = (
            "prepsubband -nobary -o {basenm}_topo_search_mask "
            "-mask {basenm}_rfifind.mask -lodm {low_dm} "
            "-dmstep {dm_step} -numdms {ndms:d} *search*.fil".format(**kwargs)
        )
        ret = execute(cmd, out=outfile, err=errfile)
        cmd = (
            "prepsubband -nobary -o {basenm}_topo_search_nomask "
            "-lodm {low_dm} "
            "-dmstep {dm_step} -numdms {ndms:d} *search*.fil".format(**kwargs)
        )
        ret = execute(cmd, out=outfile, err=errfile)
        ...
        if zero_dm:
            ...
            ### TODO: CHANGE TO .fits WHEN DIGIFITS IS WORKING
            cmd = (
                "prepsubband -nobary -zerodm -o {basenm}_topo_zerodm_search_mask "
                "-mask {basenm}_rfifind.mask -lodm {low_dm} "
            ...
            ret = execute(cmd, out=outfile, err=errfile)
            cmd = (
                "prepsubband -nobary -zerodm -o {basenm}_topo_zerodm_search_nomask "
                "-lodm {low_dm} "
                "-dmstep {dm_step} -numdms {ndms:d} "
                "*search*.fil".format(**kwargs)
            )
    ...
    for gs in glob_strings:
        cands = sifting.read_candidates(glob.glob("*" + gs + "_search*ACCEL_50"))
        if len(cands):
    ...
    if parfile is not None:
        ...
        ### TODO: CHANGE TO .fits WHEN DIGIFITS IS WORKING
        cmd = "prepfold -noxwin -o {basenm} -timing {parfile} " "*search*.fil".format(
            **kwargs
        )
        ret = execute(cmd, out=outfile, err=errfile)
        ...
        # pulsar
        cmd = (
            "prepdata -o {basenm}_bary_mask_DM{dm:.2f} -dm {dm} "
            "-mask {basenm}_rfifind.mask *search*.fil".format(**kwargs)
        )
        ret = execute(cmd, out=outfile, err=errfile)
        cmd = (
            "prepdata -o {basenm}_bary_nomask_DM{dm:.2f} -dm {dm} "
            "*search*.fil".format(**kwargs)
        )
        ret = execute(cmd, out=outfile, err=errfile)

        cmd = (
            "prepfold -noxwin -timing {parfile} "
        ...
        if zero_dm:
            ...
            ### TODO: CHANGE TO .fits WHEN DIGIFITS IS WORKING
            cmd = (
                "prepfold -noxwin -zerodm -o {basenm}_zerodm -timing "
            ...
            cmd = (
                "prepdata -zerodm -o {basenm}_bary_zerodm_mask_DM{dm:.2f} "
                "-dm {dm} -mask {basenm}_rfifind.mask "
                "*search*.fil".format(**kwargs)
            )
            ret = execute(cmd, out=outfile, err=errfile)
            cmd = (
                "prepdata -zerodm -o {basenm}_bary_zerodm_nomask_DM{dm:.2f} "
                "-dm {dm} "
                "*search*.fil".format(**kwargs)
            )
            ret = execute(cmd, out=outfile, err=errfile)
        ...
        if make_topocentric:
            ...
            ### TODO: CHANGE TO .fits WHEN DIGIFITS IS WORKING
            cmd = (
                "prepdata -nobary -o {basenm}_topo_mask_DM{dm:.2f} -dm {dm} "
                "-mask {basenm}_rfifind.mask *search*.fil".format(**kwargs)
            )
            ret = execute(cmd, out=outfile, err=errfile)
            cmd = (
                "prepdata -nobary -o {basenm}_topo_nomask_DM{dm:.2f} -dm {dm} "
                "*search*.fil".format(**kwargs)
            )
            ret = execute(cmd, out=outfile, err=errfile)
...
def do_fold(
    ...
    if nchan is not None:
        cmd = (
            "dspsr -t 8 -O {basenm}_fold -a psrfits -e fits -F {nchan}:D "
            "-D {dm} -d 4 -b {nbin} -E {t2parfile} -A -nsub 1 -L 1.0 "
            "-U 8192 {infiles}".format(**kwargs)
        )
    else:
        cmd = (
            "dspsr -t 8 -O {basenm}_fold -a psrfits -e fits "
            "-D {dm} -d 4 -b {nbin} -E {t2parfile} -A -nsub 1 -L 1.0 "
            "-U 8192 {infiles}".format(**kwargs)
        )
    ret = execute(cmd, out=outfile, err=errfile)
    cmd = "psredit -m -c freq=1500.0,rcvr:hand=-1,be:phase=-1,rcvr:sa=-45deg " "*fold*"
    ret = execute(cmd, out=outfile, err=errfile)
    cmd = "psradd -o {basenm}_fold_sum.fits -E {t2parfile} " "*fold_????.fits*".format(
        **kwargs
    )
    ret = execute(cmd, out=outfile, err=errfile)
    ...
    if cal_psr is not None:
        ...
        cmd = (
            "dspsr -t 8 -O {cal_basenm}_cal -a psrfits -e fits "
            "-F {nchan}:D -D 0.0 -K -d 4 -b {nbin} -c 0.04 -p 0 -A "
            "-nsub 1 -L 1.0 -U 8192 {cal_psr}".format(**kwargs)
        )
        ret = execute(cmd, out=outfile, err=errfile)
        cmd = "pam -m --type PolnCal {cal_basenm}_cal_0001.fits".format(**kwargs)
        ret = execute(cmd, out=outfile, err=errfile)
        cmd = (
            "psredit -m -c freq=1500.0,rcvr:hand=-1,be:phase=-1,"
        ...
        if fluxcal is None and fluxcal_on is not None and fluxcal_off is not None:
            ...
            cmd = (
                "dspsr -t 8 -O {fluxcal_off_basenm}_cal -a psrfits "
        ...
        cmd = (
            "psradd -o {basenm}_fold_sum.calib -E {t2parfile} "
    ...
    cmd = (
        "pat -F -f princeton -s {basenm}_fold_sum.std *fold*.fscr "
