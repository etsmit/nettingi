...
def translate_header(psrfits_file,sum_pols=False):
    ...
    fil_header["nifs"] = subint_hdr['NPOL'] if not sum_pols else 1

    return fil_header
                         
def main(fits_fn, outfn, nbits, \
            apply_weights, apply_scales, apply_offsets,sum_pols):
    start = time.time()
    psrfits_file = psrfits.PsrfitsFile(fits_fn)

    fil_header = translate_header(psrfits_file,sum_pols=sum_pols) 
...
if __name__=='__main__':
    ...
    parser.add_option("--sumpols", dest='sum_pols', \
                      default=False, action="store_true", \
                      help="Sum polarizations before writing filterbank file.")
    ...
    main(fits_fn, outfn, options.nbits, options.apply_weights, \
            options.apply_scales, options.apply_offsets,options.sum_pols)
